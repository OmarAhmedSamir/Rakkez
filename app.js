...
/* =========================================================
   RESET ALL STATS (KEEP STREAK)

   Wipes lifetime + daily Focus time, Sessions and Daily
   Goal progress back to 0.

   streak and lastFocusDate are deliberately left untouched
   so the user doesn't lose their streak just for wanting a
   clean slate on their numbers.
   ========================================================= */
...
function resetAllStats() {
    ...
    console.log(
        "RakkeZ: all stats reset (streak kept):",
        stats.streak
    );
...
/* =========================================================
   RESET TIMER
   ---------------------------------------------------------
   Removed: this whole reset-confirmation subsystem
   (resetCurrentSegment/resetFullSession/openResetConfirmation/
   createResetConfirmation/initializeResetSystem) was never
   called by anything in the page. The real reset flow lives
   inline in index.html (openReset/closeReset/directReset),
   wired to the #resetBtn button.
   =========================================================
...
/* =========================================================
   THEME + AMBIENT
   ---------------------------------------------------------
   Fully handled by themes.js (window.rakkezTheme).
   No bridge needed here.
   =========================================================
...
document.addEventListener(
    ...
    e => {
        ...
        if (
            ...
            /*
             * The reset flow itself lives in
             * index.html (openReset/closeReset).
             * Just trigger the same button a
             * real click would.
             */
            const resetTrigger =
                $("resetBtn");
...
/* =========================================================
   THEME SYSTEM
   ---------------------------------------------------------
   Removed: this duplicated (and conflicted with) the
   working theme system in themes.js (window.rakkezTheme).
   That file owns theme state, storage, and the header
   button. Nothing here should re-implement it.
   =========================================================
...
async function init() {
    ...
    /*
     * Theme + ambient are fully owned and initialized
     * by themes.js (window.rakkezTheme) on its own
     * DOMContentLoaded listener. Nothing to do here.
     */
